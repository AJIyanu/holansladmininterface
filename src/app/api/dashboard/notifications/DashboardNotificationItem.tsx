...
import { Bell, CircleAlert, ListTodo } from "lucide-react";
...
function formatRelativeTime(value: string): string {
  ...
  const difference = createdAt.getTime() - Date.now();
  ...
  if (absoluteDifference < 24 * 60 * 60_000) {
    return formatter.format(Math.round(difference / (60 * 60_000)), "hour");
  }
...
function getNotificationIcon(notification: DashboardNotification) {
  ...
  const severity = notification.severity.trim().toUpperCase();
  ...
  if (
    category === "task" ||
    notification.notification_type.toLowerCase().includes("task")
  ) {
    return ListTodo;
  }
    ...
    >
      ...
      <div
        className={cn(
          ...
          !notification.is_read && "border-primary/30 bg-primary/10",
        )}
      >
